...
const columnasDepreciaicion = [
  { field: "idDepreciacion", headerName: "ID", width: 150 },
  { field: "tasa", headerName: "Tasa", width: 120, type: "string" },
  {
  ...
  { field: "fecha", headerName: "Fecha", width: 120, type: "string" },
...
const InventarioInmuebleDepreciacion = () => {
  const { handleError } = useSistema();
  const { dialogoManager, formManager } = useInventarioInmueble();
  ...
  const [expanded, setExpanded] = useState(tieneRegistros ? "resumen" : null);

  const handleChange = (panel) => (event, isExpanded) => {
    setExpanded(isExpanded ? panel : false);
  };
  const tituloDialogo = "Depreciación del Bien Inmueble";
  ...
  return (
    ...
    >
      ...
      {!cargando && !tieneRegistros && (
        <Mensaje mensaje="No se han encontrado registros de depreciación para este bien inmueble" />
      )}
    </DialogoEmergente>
  );
};

export default InventarioInmuebleDepreciacion;

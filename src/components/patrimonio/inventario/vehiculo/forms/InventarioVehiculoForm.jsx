...
import { vehiculoAltaValidacion } from "../../../../../settings/validacionConfig";
...
import { getBienVehiculo } from "../../../../../services/patrimonio";
import InventarioVehiculoDatoBienForm from "./InventarioVehiculoDatoBienForm";
import InventarioVehiculoDatoAdquisicionForm from "./InventarioVehiculoDatoAdquisicionForm";
import InventarioVehiculoDatoGeneralForm from "./InventarioVehiculoDatoGeneralForm";
import InventarioVehiculoCaracteristicasForm from "./InventarioVehiculoCaracteristicasForm";
import InventarioVehiculoResponsableForm from "./InventarioVehiculoResponsableForm";
...
const InventarioVehiculoForm = () => {
  ...
  const tituloDialogo = getTituloDialogo("Bien Maquinaria y Vehículo");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getCaracteristicas(),
      getResponsables(),
      getBienVehiculo({ idBien: filaSeleccionada[0] }),
    ])
      .then(
        ([
          familiasData,
  ...
  useEffect(() => {
    ...
    if (detalleAlta) {
      ...
      setValue(CAMPOS_INVENTARIO_VEHICULO.BIEN, detalleAlta.idBien ?? "");
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_ALTA,
        detalleAlta.fechaAlta ? dayjs(detalleAlta.fechaAlta) : null
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_BAJA,
        detalleAlta.fechaBaja ? dayjs(detalleAlta.fechaBaja) : null
      );
      setValue(CAMPOS_INVENTARIO_VEHICULO.MOTIVO_BAJA, detalleAlta.motivoBaja);
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FOLIO_BIEN,
        detalleAlta.folioBien ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.CUENTA_POR_PAGAR,
        detalleAlta.cuentaPorPagar ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.SUSTITUYE_BV,
        detalleAlta.sustituyeBV ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ANIO_EMICION,
        detalleAlta.anioEmicion ?? ""
      );
      //
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NUMERO_PLACA,
        detalleAlta.numeroPlaca ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NUMERO_MOTOR,
        detalleAlta.numeroMotor ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ANIO_MODELO,
        detalleAlta.anioModelo ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NUMERO_ECONOMICO,
        detalleAlta.numeroEconomico ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_CLAVE,
        claves.find((e) => e.id === detalleAlta.idClave)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_LINEA,
        lineas.find((e) => e.id === detalleAlta.idLinea)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_VERSION,
        versiones.find((e) => e.id === detalleAlta.idVersion)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_CLASE,
        clases.find((e) => e.id === detalleAlta.idClase)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_TIPO,
        tipos.find((e) => e.id === detalleAlta.idTipo)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ID_COMBUSTIBLE,
        combustibles.find((e) => e.id === detalleAlta.idCombustible)
      );

      setValue(
        CAMPOS_INVENTARIO_VEHICULO.SOLICITUD,
        detalleAlta.idSolicitud ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NUMERO_BIENES,
        detalleAlta.numeroBienes ?? ""
      );
      setValue(CAMPOS_INVENTARIO_VEHICULO.PARTIDA, detalleAlta.partida ?? "");
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.PARTIDA_ESPECIFICA,
        detalleAlta.partidaEspecifica ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.CUENTA_ACTIVO,
        detalleAlta.referenciaActivo ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.CUENTA_ACTUALIZACION,
        detalleAlta.referenciaActualizacion ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FAMILIA,
        familias.find((e) => e.id === detalleAlta.idFamilia)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.SUBFAMILIA,
        subfamilias.find((e) => e.id === detalleAlta.idSubfamilia)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.BMS,
        bms.find((bien) => bien.id === detalleAlta.idBms)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.DESCRIPCION,
        detalleAlta.descripcion ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.UNIDAD_ADMINISTRATIVA,
        unidadesAdministrativas.find(
          (e) => e.id === detalleAlta.nivelUnidadAdministrativa
        )
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.REQUISICION,
        detalleAlta.requisicion ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ORDEN_COMPRA,
        detalleAlta.ordenCompra ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.TIPO_ADQUISICION,
        tiposAdquisicion.find((e) => e.id === detalleAlta.idTipoAdquisicion)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NO_SERIES,
        detalleAlta.noSeries ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FOLIO_ANTERIOR,
        detalleAlta.folioAnterior ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.NO_LICITACION,
        detalleAlta.noLicitacion ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_LICITACION,
        detalleAlta.fechaLicitacion ? dayjs(detalleAlta.fechaLicitacion) : null
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.OBSERVACION_LICITACION,
        detalleAlta.observacionLicitacion
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.ESTADO_FISICO,
        estadosFisicos.find((e) => e.id === detalleAlta.idEstadoFisico)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.MARCA,
        marcas.find((e) => e.id === detalleAlta.idMarca)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.COLOR,
        colores.find((e) => e.id === detalleAlta.idColor)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FOLIO_FACTURA,
        detalleAlta.folioFactura ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_FACTURA,
        dayjs(detalleAlta.fechaFactura)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.PRECIO_UNITARIO,
        detalleAlta.precioUnitario ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_COMPRA,
        dayjs(detalleAlta.fechaCompra)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.DIAS_GARANTIA,
        detalleAlta.diasGarantia ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.VIDA_UTIL,
        detalleAlta.vidaUtil ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.FECHA_INICIO_USO,
        dayjs(detalleAlta.fechaInicioUso)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.PRECIO_DESECHABLE,
        detalleAlta.precioDesechable ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.OBSERVACION_BIEN,
        detalleAlta.observacionBien ?? ""
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.UBICACION,
        ubicaciones.find((e) => e.id === detalleAlta.idUbicacion)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.MUNICIPIO,
        municipios.find((e) => e.id === detalleAlta.idMunicipio)
      );
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.CARACTERISTICAS,
        caracteristicasSelect
      );
      setValue(CAMPOS_INVENTARIO_VEHICULO.AUX_CARACTERISTICA, filasTabla);
      setValue(CAMPOS_INVENTARIO_VEHICULO.RESPONSABLES, responsablesSelect);
      setValue(
        CAMPOS_INVENTARIO_VEHICULO.OBSERVACION_RESPONSABLE,
  ...
  const getStepContent = (step) => {
    switch (step) {
      case formDatosBien:
        return (
          <InventarioVehiculoDatoBienForm
      ...
      case formDatosAdquisicion:
        return (
          <InventarioVehiculoDatoAdquisicionForm
            formManager={formManager}
            complementos={complementos}
          />
        );
      case formDatosGenerales:
        return (
          <InventarioVehiculoDatoGeneralForm
            formManager={formManager}
            complementos={complementos}
          />
        );
      case formCaracteristicas:
        return (
          <InventarioVehiculoCaracteristicasForm
            formManager={formManager}
            complementos={complementos}
          />
        );
      default:
        return (
          <InventarioVehiculoResponsableForm
            formManager={formManager}
            complementos={complementos}

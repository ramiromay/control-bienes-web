...
import { CAMPOS_SOLICITUD_INMUEBLE } from "../../../../../../settings/formConfig";

const AdministradorInmuebleSolicitudForm = () => {
  const {
    ID_SOLICITUD_MUEBLE,
    SOLICITANTE,
    OBSERVACIONES,
  ...
  const tituloDialogo = getTituloDialogo("Solicitud Bien Inmueble");
      ...
      .then(
        ([
          ...
          const tiposTramites = mapArray(
            tiposTramitesData.filter((e) => e.idSubModulo === 2),
            compTipoTramiteMappingRules
          );
  ...
  useEffect(() => {
    if (solicitud) {
      ...
      setValue(DOCUMENTO, documentoReferencia);
...
export default AdministradorInmuebleSolicitudForm;

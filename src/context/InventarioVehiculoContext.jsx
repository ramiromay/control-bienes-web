...
const InventarioVehiculoContext = createContext();

export const useInventarioVehiculo = () => {
  if (!InventarioVehiculoContext) {
    throw new Error(
      "useInventarioMueble debe ser usado dentro de InventarioMuebleProvider"
    );
  }
  return useContext(InventarioVehiculoContext);
};
...
export const InventarioVehiculoProvider = ({ children }) => {
  const {
    handleError,
    handleIniciarCarga,
  ...
  const handleGetBienesVehiculos = () => {
    ...
    trigger()
      .then((valido) => {
        ...
        return getBienesVehiculos(parametrosFiltro);
      })
      .then((bienes) => {
        setTabla((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    handleChangeModulo({
      idSubModulo: IDS_SUBMODULOS.INVENTARIO_BIENES_VEHICULARES,
    });
    Promise.all([
      getPeriodos(),
      getArticulos({ tipoBien: 3 }),
  ...
  return (
    <InventarioVehiculoContext.Provider
      value={{
        tabla,
        unidadAdministrativa,
        tipoBien,

...
const InventarioInmuebleContext = createContext();

export const useInventarioInmueble = () => {
  if (!InventarioInmuebleContext) {
    throw new Error(
      "useInventarioMueble debe ser usado dentro de InventarioMuebleProvider"
    );
  }
  return useContext(InventarioInmuebleContext);
};
...
export const InventarioInmuebleProvider = ({ children }) => {
  const {
    handleError,
    handleIniciarCarga,
  ...
  const handleGetBienesInmuebles = () => {
    ...
    trigger()
      .then((valido) => {
        ...
        return getBienesInmuebles(parametrosFiltro);
      })
      .then((bienes) => {
        setTabla((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    handleChangeModulo({
      idSubModulo: IDS_SUBMODULOS.INVENTARIO_BIENES_INMUEBLES,
    });
    Promise.all([
      getPeriodos(),
      getArticulos({ tipoBien: 4 }),
  ...
  return (
    <InventarioInmuebleContext.Provider
      value={{
        tabla,
        unidadAdministrativa,
        tipoBien,
